...
public partial class LgwUserControls_MetaContentEditor : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            //load radiobuttonlist recordstatus             
            ListItem item = new ListItem();
            item.Text = String.Format("<span style=\"width:50px\">{0}</span>", Resources.strings.Trash_Text);
            item.Value = "-1";
            item.Selected = true;
            radioRecordStatus.Items.Add(item);
            item = new ListItem();
            item.Text = String.Format("<span style=\"width:50px\">{0}</span>", Resources.strings.Pendding_Text);
            item.Value = "0";
            radioRecordStatus.Items.Add(item);
            item = new ListItem();
            item.Text = String.Format("<span style=\"width:50px\">{0}</span>", Resources.strings.Published_Text);
            item.Value = "1";
            radioRecordStatus.Items.Add(item);
            //load access level
            item = new ListItem();
            item.Text = Resources.strings.Public_Text;
            item.Value = "0";
            item.Selected = true;
            dropAccessLevels.Items.Add(item);

            item = new ListItem();
            item.Text = Resources.strings.Registered_Text;
            item.Value = "1";
            dropAccessLevels.Items.Add(item);

            item = new ListItem();
            item.Text = Resources.strings.Special_Text;
            item.Value = "2";
            dropAccessLevels.Items.Add(item);
            ...
            item = new ListItem();
            item.Text = Resources.strings.English_Text;
            item.Value = "en";
            dropLanguages.Items.Add(item);
    ...
    protected void edit_MetaContent(Int32 iMetaContentId)
    {
        ...
        string sTemplateName=LegoWebAdmin.BusLogic.Categories.get_CATEGORY_TEMPLATE_NAME(int.Parse(dropCategories.SelectedValue.ToString()));
        ...
        if (_MetaContent.AccessLevel >= 2)//special permission
        {
            cblRoles.Visible = true;
            //get roles in formation 
            string[] accessRoles = LegoWebAdmin.BusLogic.MetaContents.get_ACCESS_ROLES(iMetaContentId);
            if (accessRoles != null && accessRoles.Length > 0)
            {
                for (int i = 0; i < accessRoles.Length; i++)
                {
    ...
    protected void dropCategories_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (dropCategories.SelectedValue != null)
        {
            ...
            {                
                ...
                litErrorSpaceHolder.Text = String.Format(errorFomat, "You are not authorize to update this category meta contents!");
            }
    ...
    /// <summary>
    /// Check if current user can update content to this category
    /// </summary>
