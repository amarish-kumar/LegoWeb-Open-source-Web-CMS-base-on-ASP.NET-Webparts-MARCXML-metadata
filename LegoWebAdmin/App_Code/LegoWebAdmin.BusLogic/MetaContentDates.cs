// ----------------------------------------------------------------------
// <copyright file="MetaContentDates.cs" company="HIENDAI SOFTWARE COMPANY">
...
namespace LegoWebAdmin.BusLogic
{
    ...
    /// </summary>
    public static class MetaContentDates
    {

        public static Int32 insert_META_CONTENT_DATES(int iMETA_CONTENT_ID, int iTAG, int iTAG_INDEX,string sSUBFIELD_CODE,string sSUBFIELD_VALUE, bool bIS_PUBLIC, int iACCESS_LEVEL, string sCREATED_USER)
        {
            ...
            try
            {
                ...
                strCommandName = "sp_LEGOWEB_META_CONTENT_DATES_INSERT";
                objCommand = new SqlCommand(strCommandName, connection);
                objCommand.CommandType = CommandType.StoredProcedure;
                //Set the Parameters

                Int32 iMETA_CONTENT_DATE_ID = 0;
                ...
                objParam = objCommand.Parameters.Add(new SqlParameter("@_SUBFIELD_VALUE", SqlDbType.NVarChar,30));
                objParam.Direction = ParameterDirection.Input;
                ...
                objParam = objCommand.Parameters.Add(new SqlParameter("@_META_CONTENT_DATE_ID", SqlDbType.Int));
                objParam.Direction = ParameterDirection.Output;

                connection.Open();
                objCommand.ExecuteNonQuery();
                //'Get the Out parameter from the Stored Procedure
                iMETA_CONTENT_DATE_ID = Convert.ToInt32((objCommand.Parameters["@_META_CONTENT_DATE_ID"].Value.ToString()));

                connection.Close();
                return iMETA_CONTENT_DATE_ID;
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public static void update_META_CONTENT_DATES(int iMETA_CONTENT_DATE_ID,int iTAG, int iTAG_INDEX,string sSUBFIELD_CODE, string sSUBFIELD_VALUE , bool bIS_PUBLIC, int iACCESS_LEVEL, string sMODIFIED_USER)
        {
            string connStr = ConfigurationManager.ConnectionStrings["LEGOWEBDB"].ConnectionString;
            SqlConnection connection = new SqlConnection(connStr);
            try
            {
                ...
                strCommandName = "sp_LEGOWEB_META_CONTENT_DATES_UPDATE";
                objCommand = new SqlCommand(strCommandName, connection);
                objCommand.CommandType = CommandType.StoredProcedure;
                //Set the Parameters
                ...
                objParam = objCommand.Parameters.Add(new SqlParameter("@_META_CONTENT_DATE_ID", SqlDbType.Int));
                objParam.Direction = ParameterDirection.Input;
                objParam.Value = iMETA_CONTENT_DATE_ID;
        ...
        public static bool is_META_CONTENT_DATE_EXIST(Int32 iID)
        {
            String connStr = ConfigurationManager.ConnectionStrings["LEGOWEBDB"].ConnectionString;
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                try
                {
                    SqlCommand cmdReader = new SqlCommand("SELECT META_CONTENT_DATE_ID FROM LEGOWEB_META_CONTENT_DATES WHERE META_CONTENT_DATE_ID=" + iID.ToString(), conn);
                    conn.Open();
                    SqlDataReader reader = cmdReader.ExecuteReader();
                    if (reader.HasRows)
                    {
                        reader.Read();
                        Int32 retID = (Int32)reader["META_CONTENT_DATE_ID"];
                        conn.Close();
                        return true;
                    }
                    else
