...
CREATE procedure sp_LEGOWEB_CATEGORIES_ADDUPDATE
		...
		@_CATEGORY_ALIAS nvarchar(250)=NULL,	
	  ...
	  begin    
	     ...
	     begin    
			...
			set
				...
				CATEGORY_ALIAS=@_CATEGORY_ALIAS,	
...
(         
	select              
		...
		Category_Vi_Title + '(' + cast((Select COUNT(META_CONTENT_ID) From LEGOWEB_META_CONTENTS Where LEGOWEB_META_CONTENTS.RECORD_STATUS=1 AND LEGOWEB_META_CONTENTS.CATEGORY_ID=c.Category_Id) as nvarchar(20)) + ')' as "@Category_Vi_Title",             
		Category_En_Title + '(' + cast((Select COUNT(META_CONTENT_ID) From LEGOWEB_META_CONTENTS Where LEGOWEB_META_CONTENTS.RECORD_STATUS=1 AND LEGOWEB_META_CONTENTS.CATEGORY_ID=c.Category_Id) as nvarchar(20)) + ')' as "@Category_En_Title",             
...
BEGIN
	...
	AS
	(
	SELECT CATEGORY_ID, SECTION_ID, PARENT_CATEGORY_ID, CATEGORY_VI_TITLE,CATEGORY_EN_TITLE,CATEGORY_ALIAS, MENU_ID,IS_PUBLIC,CATEGORY_TEMPLATE_NAME, 0 AS Lv, CAST('\' + LTRIM(CATEGORY_ID) + '\' AS VARCHAR(MAX)) AS DIRECTION_PATH, CAST('\' + right(replicate('0', 2) + convert(varchar(10), ORDER_NUMBER), 2) + '\' AS VARCHAR(MAX)) AS ORDER_PATH,ORDER_NUMBER, ADMIN_LEVEL, ADMIN_ROLES
	FROM LEGOWEB_CATEGORIES
	WHERE ((@_CATEGORY_ID IS NULL AND ((@_PARENT_CATEGORY_ID IS NULL AND ((@_SECTION_ID IS NULL) OR (SECTION_ID=@_SECTION_ID AND PARENT_CATEGORY_ID=0))) OR (PARENT_CATEGORY_ID=@_PARENT_CATEGORY_ID))) OR (CATEGORY_ID=@_CATEGORY_ID))	  		  	
	UNION ALL
	SELECT c.CATEGORY_ID,c.SECTION_ID, c.PARENT_CATEGORY_ID, c.CATEGORY_VI_TITLE,c.CATEGORY_EN_TITLE,c.CATEGORY_ALIAS,c.MENU_ID,c.IS_PUBLIC,c.CATEGORY_TEMPLATE_NAME,p.Lv + 1 AS Lv, p.DIRECTION_PATH + CAST(LTRIM(c.CATEGORY_ID) + '\' AS VARCHAR(MAX)) AS DIRECTION_PATH,p.ORDER_PATH + CAST(right(replicate('0', 2) + convert(varchar(10), c.ORDER_NUMBER), 2) + '\' AS VARCHAR(MAX)) AS ORDER_PATH,c.ORDER_NUMBER,  c.ADMIN_LEVEL, c.ADMIN_ROLES
	FROM LEGOWEB_CATEGORIES AS c INNER JOIN hierarchy AS p ON c.PARENT_CATEGORY_ID = p.CATEGORY_ID
	)
	SELECT DISTINCT TOP(@_SELECT_ROW_NUMBER) hierarchy.CATEGORY_ID,hierarchy.PARENT_CATEGORY_ID,hierarchy.SECTION_ID,((REPLICATE(@_TAB_CHARS, Lv * 1)) + hierarchy.CATEGORY_VI_TITLE) AS CATEGORY_VI_TITLE,((REPLICATE(@_TAB_CHARS, Lv * 1)) + hierarchy.CATEGORY_EN_TITLE) AS CATEGORY_EN_TITLE,CATEGORY_ALIAS,hierarchy.IS_PUBLIC, hierarchy.CATEGORY_TEMPLATE_NAME,hierarchy.DIRECTION_PATH,hierarchy.ORDER_PATH,hierarchy.ORDER_NUMBER, hierarchy.ADMIN_LEVEL, hierarchy.ADMIN_ROLES,LEGOWEB_SECTIONS.SECTION_VI_TITLE,LEGOWEB_SECTIONS.SECTION_EN_TITLE,LEGOWEB_MENUS.MENU_VI_TITLE,LEGOWEB_MENUS.MENU_EN_TITLE
...
CREATE procedure [dbo].[sp_LEGOWEB_META_CONTENTS_INSERT]
	...
	@_META_CONTENT_ALIAS nvarchar (250)=NULL,
	@_CREATED_USER nvarchar (30),
	@_RECORD_STATUS smallint=0,
...
BEGIN
	...
	META_CONTENT_ALIAS=@_META_CONTENT_ALIAS,
...
BEGIN
	...
	SELECT COUNT(DISTINCT LEGOWEB_META_CONTENTS.META_CONTENT_ID) FROM LEGOWEB_META_CONTENTS INNER JOIN hierarchy ON LEGOWEB_META_CONTENTS.CATEGORY_ID=hierarchy.CATEGORY_ID AND LEGOWEB_META_CONTENTS.RECORD_STATUS>=0 
END
GO
...
BEGIN
	...
	AS
	(
		SELECT CATEGORY_ID,CATEGORY_VI_TITLE,CATEGORY_EN_TITLE
		FROM LEGOWEB_CATEGORIES
		WHERE (((@_ROOT_CATEGORY_ID IS NULL) AND ((@_SECTION_ID IS NULL) OR (SECTION_ID=@_SECTION_ID AND PARENT_CATEGORY_ID=0))) OR (CATEGORY_ID=@_ROOT_CATEGORY_ID))
		UNION ALL
		SELECT c.CATEGORY_ID,c.CATEGORY_VI_TITLE,c.CATEGORY_EN_TITLE
		FROM LEGOWEB_CATEGORIES AS c INNER JOIN hierarchy AS p ON c.PARENT_CATEGORY_ID = p.CATEGORY_ID
	)
	SELECT DISTINCT TOP(@_SELECT_ROW_NUMBER) LEGOWEB_META_CONTENTS.*,hierarchy.CATEGORY_VI_TITLE,hierarchy.CATEGORY_EN_TITLE 
	FROM LEGOWEB_META_CONTENTS INNER JOIN hierarchy ON LEGOWEB_META_CONTENTS.CATEGORY_ID=hierarchy.CATEGORY_ID AND LEGOWEB_META_CONTENTS.RECORD_STATUS>=0
	ORDER BY LEGOWEB_META_CONTENTS.MODIFIED_DATE DESC	
END
GO

if exists (select * from sysobjects where id = object_id('dbo.sp_LEGOWEB_META_CONTENTS_TRASH') and sysstat & 0xf = 4)   drop procedure dbo.sp_LEGOWEB_META_CONTENTS_TRASH    
GO
CREATE procedure sp_LEGOWEB_META_CONTENTS_TRASH
as
BEGIN
	SELECT DISTINCT LEGOWEB_META_CONTENTS.*,LEGOWEB_CATEGORIES.CATEGORY_VI_TITLE,LEGOWEB_CATEGORIES.CATEGORY_EN_TITLE 
	FROM LEGOWEB_META_CONTENTS INNER JOIN LEGOWEB_CATEGORIES ON LEGOWEB_META_CONTENTS.CATEGORY_ID=LEGOWEB_CATEGORIES.CATEGORY_ID AND LEGOWEB_META_CONTENTS.RECORD_STATUS<0
	ORDER BY LEGOWEB_META_CONTENTS.MODIFIED_DATE DESC	
END
GO
...
CREATE procedure [dbo].[sp_LEGOWEB_META_CONTENTS_GET_FOR_XML]
	@_META_CONTENT_ID int,
	@_SCOPE smallint =0--can bee define more to specific information scope to get
as
BEGIN
if(@_SCOPE=0)
	BEGIN
