...
BEGIN
	...
	AS
	(
	SELECT CATEGORY_ID, SECTION_ID, PARENT_CATEGORY_ID, CATEGORY_VI_TITLE, MENU_ID,IS_PUBLIC,CATEGORY_TEMPLATE_NAME, 0 AS Lv, CAST('\' + LTRIM(CATEGORY_ID) + '\' AS VARCHAR(MAX)) AS DIRECTION_PATH, CAST('\' + right(replicate('0', 2) + convert(varchar(10), ORDER_NUMBER), 2) + '\' AS VARCHAR(MAX)) AS ORDER_PATH,ORDER_NUMBER, ADMIN_LEVEL, ADMIN_ROLES
	FROM LEGOWEB_CATEGORIES
	WHERE ((@_CATEGORY_ID IS NULL AND ((@_PARENT_CATEGORY_ID IS NULL AND ((@_SECTION_ID IS NULL) OR (SECTION_ID=@_SECTION_ID AND PARENT_CATEGORY_ID=0))) OR (PARENT_CATEGORY_ID=@_PARENT_CATEGORY_ID))) OR (CATEGORY_ID=@_CATEGORY_ID))	  		  	
	UNION ALL
	SELECT c.CATEGORY_ID,c.SECTION_ID, c.PARENT_CATEGORY_ID, c.CATEGORY_VI_TITLE,c.MENU_ID,c.IS_PUBLIC,c.CATEGORY_TEMPLATE_NAME,p.Lv + 1 AS Lv, p.DIRECTION_PATH + CAST(LTRIM(c.CATEGORY_ID) + '\' AS VARCHAR(MAX)) AS DIRECTION_PATH,p.ORDER_PATH + CAST(right(replicate('0', 2) + convert(varchar(10), c.ORDER_NUMBER), 2) + '\' AS VARCHAR(MAX)) AS ORDER_PATH,c.ORDER_NUMBER,  c.ADMIN_LEVEL, c.ADMIN_ROLES
	FROM LEGOWEB_CATEGORIES AS c INNER JOIN hierarchy AS p ON c.PARENT_CATEGORY_ID = p.CATEGORY_ID
	)
	SELECT DISTINCT TOP(@_SELECT_ROW_NUMBER) hierarchy.CATEGORY_ID,hierarchy.PARENT_CATEGORY_ID,hierarchy.SECTION_ID,((REPLICATE(@_TAB_CHARS, Lv * 1)) + hierarchy.CATEGORY_VI_TITLE) AS CATEGORY_VI_TITLE,hierarchy.IS_PUBLIC, hierarchy.CATEGORY_TEMPLATE_NAME,hierarchy.DIRECTION_PATH,hierarchy.ORDER_PATH,hierarchy.ORDER_NUMBER, hierarchy.ADMIN_LEVEL, hierarchy.ADMIN_ROLES,LEGOWEB_SECTIONS.SECTION_VI_TITLE,LEGOWEB_MENUS.MENU_VI_TITLE
