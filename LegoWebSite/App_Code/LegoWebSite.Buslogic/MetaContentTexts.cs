...
namespace LegoWebSite.Buslgic
{
    ...
    /// </summary>
    public static class MetaContentTexts
    {

        public static Int32 insert_META_CONTENT_TEXTS(int iMETA_CONTENT_ID, int iTAG, int iTAG_INDEX,string sSUBFIELD_CODE,string sSUBFIELD_VALUE, bool bIS_PUBLIC, int iACCESS_LEVEL, string sCREATED_USER)
        {
            ...
            try
            {
                ...
                strCommandName = "sp_LEGOWEB_META_CONTENT_TEXTS_INSERT";
                objCommand = new SqlCommand(strCommandName, connection);
                objCommand.CommandType = CommandType.StoredProcedure;
                //Set the Parameters

                Int32 iMETA_CONTENT_TEXT_ID = 0;
                ...
                objParam = objCommand.Parameters.Add(new SqlParameter("@_SUBFIELD_VALUE", SqlDbType.NVarChar,400));
                objParam.Direction = ParameterDirection.Input;
                ...
                objParam = objCommand.Parameters.Add(new SqlParameter("@_META_CONTENT_TEXT_ID", SqlDbType.Int));
                objParam.Direction = ParameterDirection.Output;

                connection.Open();
                objCommand.ExecuteNonQuery();
                //'Get the Out parameter from the Stored Procedure
                iMETA_CONTENT_TEXT_ID = Convert.ToInt32((objCommand.Parameters["@_META_CONTENT_TEXT_ID"].Value.ToString()));

                connection.Close();
                return iMETA_CONTENT_TEXT_ID;
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public static void update_META_CONTENT_TEXTS(int iMETA_CONTENT_TEXT_ID, int iTAG, int iTAG_INDEX, string sSUBFIELD_CODE, string sSUBFIELD_VALUE, bool bIS_PUBLIC, int iACCESS_LEVEL, string sMODIFIED_USER)
        {
            string connStr = ConfigurationManager.ConnectionStrings["LEGOWEBDB"].ConnectionString;
            SqlConnection connection = new SqlConnection(connStr);
            try
            {
                ...
                strCommandName = "sp_LEGOWEB_META_CONTENT_TEXTS_UPDATE";
                objCommand = new SqlCommand(strCommandName, connection);
                objCommand.CommandType = CommandType.StoredProcedure;
                //Set the Parameters
                ...
                objParam = objCommand.Parameters.Add(new SqlParameter("@_META_CONTENT_TEXT_ID", SqlDbType.Int));
                objParam.Direction = ParameterDirection.Input;
                objParam.Value = iMETA_CONTENT_TEXT_ID;
        ...
        public static bool is_META_CONTENT_TEXT_EXIST(Int32 iID)
        {
            String connStr = ConfigurationManager.ConnectionStrings["LEGOWEBDB"].ConnectionString;
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                try
                {
                    SqlCommand cmdReader = new SqlCommand("SELECT META_CONTENT_TEXT_ID FROM LEGOWEB_META_CONTENT_TEXTS WHERE META_CONTENT_TEXT_ID=" + iID.ToString(), conn);
                    conn.Open();
                    SqlDataReader reader = cmdReader.ExecuteReader();
                    if (reader.HasRows)
                    {
                        reader.Read();
                        Int32 retID = (Int32)reader["META_CONTENT_TEXT_ID"];
                        conn.Close();
                        return true;
                    }
                    else
        ...
        public static DataTable get_META_CONTENT_TEXT(int iID)
        {
            ...
            using (SqlConnection Conn = new SqlConnection(ConnString))
            {
                try
                {
                    ...
                    SqlCommand objCommand;
                    strCommandText = "SELECT * FROM LEGOWEB_META_CONTENT_TEXTS WHERE META_CONTENT_TEXT_ID=" + iID.ToString();
